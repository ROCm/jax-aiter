...
#     mxfp4|fp8|bf16 OUT_ROOT [50]
#
# Defaults reproduce the batch-9 three-leg cohort reported in the ROCm blog.
# CI must select RECIPE_PROFILE=ci_regression before overriding batch, state,
# or remat for its smaller regression operating point.
...
require_value() {
  [[ "$2" == "$3" ]] || die "$1 must be $3 for $RECIPE_PROFILE (got $2)"
}
...
RECIPE_PROFILE="${RECIPE_PROFILE:-blog_batch9}"
case "$RECIPE_PROFILE" in
  blog_batch9|ci_regression) ;;
  *) die "RECIPE_PROFILE must be blog_batch9 or ci_regression" ;;
esac
MEMFRAC="${XLA_PYTHON_CLIENT_MEM_FRACTION:-.97}"
[[ "$MEMFRAC" == ".97" ]] ||
  die "canonical memory fraction is .97; stop instead of changing it"
...
[[ "$MODEL_NAME" == "llama3.1-8b" ]] ||
  die "publication runner requires MODEL_NAME=llama3.1-8b"
FSDP="${ICI_FSDP_PARALLELISM:-8}"
SHARDY="${SHARDY:-True}"
SCAN_LAYERS="${SCAN_LAYERS:-False}"
PER_DEVICE_BATCH="${PER_DEVICE_BATCH:-9}"
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-72}"
MAX_TARGET_LENGTH="${MAX_TARGET_LENGTH:-8192}"
AUTOTUNE_LEVEL="${AUTOTUNE_LEVEL:-4}"
ENABLE_NNX="${ENABLE_NNX:-False}"
WEIGHT_DTYPE="${WEIGHT_DTYPE:-bfloat16}"
MU_DTYPE="${MU_DTYPE:-bfloat16}"
...
case "$MODE" in
  mxfp4)
    ...
    REMAT_POLICY="${REMAT_POLICY:-minimal_flash_save_fp4_wtcol}"
    USE_IOTA_EMBED="${USE_IOTA_EMBED:-False}"
    SCHEDULER_PROFILE=nvidia
    ...
    ;;
  fp8)
    LABEL="Plain FP8"; QUANTIZATION="fp8"; USE_AITER=False
    MODEL_CONTROLS=maxtext_defaults
    SR_KEY_MODE=n/a
    ATTENTION=cudnn_flash_te
    REMAT_POLICY="${REMAT_POLICY:-minimal_flash}"
    USE_IOTA_EMBED="${USE_IOTA_EMBED:-True}"
    SCHEDULER_PROFILE=utd
    COMBINE_TH_BYTES=8589934592
    TE_ATOMIC_FP32=0
    ;;
  bf16)
    ...
    SR_KEY_MODE=n/a
    ATTENTION=cudnn_flash_te
    REMAT_POLICY="${REMAT_POLICY:-minimal}"
    USE_IOTA_EMBED="${USE_IOTA_EMBED:-True}"
    SCHEDULER_PROFILE=utd
    COMBINE_TH_BYTES=8589934592
    TE_ATOMIC_FP32=0
    ;;
  *) die "bad mode '$MODE' (expected mxfp4, fp8, or bf16)" ;;
esac

if [[ "$RECIPE_PROFILE" == "blog_batch9" ]]; then
  require_value ICI_FSDP_PARALLELISM "$FSDP" 8
  require_value SHARDY "$SHARDY" True
  require_value SCAN_LAYERS "$SCAN_LAYERS" False
  require_value PER_DEVICE_BATCH "$PER_DEVICE_BATCH" 9
  require_value GLOBAL_BATCH_SIZE "$GLOBAL_BATCH_SIZE" 72
  require_value MAX_TARGET_LENGTH "$MAX_TARGET_LENGTH" 8192
  require_value AUTOTUNE_LEVEL "$AUTOTUNE_LEVEL" 4
  require_value ENABLE_NNX "$ENABLE_NNX" False
  require_value WEIGHT_DTYPE "$WEIGHT_DTYPE" bfloat16
  require_value MU_DTYPE "$MU_DTYPE" bfloat16
  require_value INIT_WEIGHTS_SEED "$INIT_WEIGHTS_SEED" 0
  case "$MODE" in
    mxfp4)
      require_value REMAT_POLICY "$REMAT_POLICY" minimal_flash_save_fp4_wtcol
      require_value USE_IOTA_EMBED "$USE_IOTA_EMBED" False
      require_value COMBINE_TH_BYTES "$COMBINE_TH_BYTES" 67108864
      ;;
    fp8)
      require_value REMAT_POLICY "$REMAT_POLICY" minimal_flash
      require_value USE_IOTA_EMBED "$USE_IOTA_EMBED" True
      ;;
    bf16)
      require_value REMAT_POLICY "$REMAT_POLICY" minimal
      require_value USE_IOTA_EMBED "$USE_IOTA_EMBED" True
      ;;
  esac
elif [[ "$MODE" != "mxfp4" ]]; then
  die "ci_regression supports only mxfp4"
fi
...
ROCM_SDK_LIBS="/usr/local/lib/python3.12/dist-packages/_rocm_sdk_core/lib:/usr/local/lib/python3.12/dist-packages/_rocm_sdk_libraries/lib"
export LD_LIBRARY_PATH="${ROCM_SDK_LIBS}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
# Match the frozen blog runner: RCCL defaults, no lo/empty-IB override.
unset NCCL_SOCKET_IFNAME GLOO_SOCKET_IFNAME NCCL_IB_HCA
unset NCCL_IB_TIMEOUT NCCL_IB_RETRY_CNT NCCL_IB_GID_INDEX
...
export NVTE_CK_IS_V3_ATOMIC_FP32="$TE_ATOMIC_FP32"
...
if [[ "$MODE" == "mxfp4" ]]; then
  ...
  if [[ "$JAX_AITER_RUNTIME" == "installed" ]]; then
    # Do not let a mounted checkout shadow the validated wheel or its MHA cache.
  ...
  export FP4_SELECT=dispatch AITER_FP4_DISPATCH=1
  export AITER_FP4_ATTN=1 AITER_FP4_MLP=1 AITER_BF16_HIPBLASLT=0
  unset AITER_FORCE_KERNEL_NAME AITER_FORCE_LOG2_K_SPLIT
  ...
  export JA_FP4_REMAT_SAVE_COL="${JA_FP4_REMAT_SAVE_COL:-wt}"
  export JA_FP4_PACK_GATEUP_AG="${JA_FP4_PACK_GATEUP_AG:-1}"
  if [[ "$RECIPE_PROFILE" == "blog_batch9" ]]; then
    require_value JA_FP4_HADAMARD_PASSES "$JA_FP4_HADAMARD_PASSES" wgrad
    require_value JA_FP4_SR_PASSES "$JA_FP4_SR_PASSES" wgrad_col
    require_value JA_FP4_DGRAD_PARTITION "$JA_FP4_DGRAD_PARTITION" gather_packed
    require_value JA_FP4_DGRAD_REUSE_FWD_COL "$JA_FP4_DGRAD_REUSE_FWD_COL" 1
    require_value JA_FP4_REMAT_SAVE_COL "$JA_FP4_REMAT_SAVE_COL" wt
    require_value JA_FP4_PACK_GATEUP_AG "$JA_FP4_PACK_GATEUP_AG" 1
  fi
  ...
  unset FP4_SELECT AITER_FP4_DISPATCH AITER_FP4_ATTN AITER_FP4_MLP AITER_BF16_HIPBLASLT
...
[[ -z "${EXTRA_XLA:-}" ]] ||
  die "EXTRA_XLA is not allowed in the publication recipe"
if [[ "$SCHEDULER_PROFILE" == "nvidia" ]]; then
  export XLA_FLAGS="--xla_gpu_enable_latency_hiding_scheduler=true --xla_gpu_enable_triton_gemm=false --xla_gpu_enable_cublaslt=True --xla_gpu_autotune_level=${AUTOTUNE_LEVEL} --xla_gpu_all_reduce_combine_threshold_bytes=${COMBINE_TH_BYTES} --xla_gpu_all_gather_combine_threshold_bytes=${COMBINE_TH_BYTES} --xla_gpu_reduce_scatter_combine_threshold_bytes=${COMBINE_TH_BYTES} --xla_gpu_enable_pipelined_all_gather=true --xla_gpu_enable_pipelined_reduce_scatter=true --xla_gpu_enable_pipelined_all_reduce=true --xla_gpu_enable_while_loop_double_buffering=false --xla_gpu_enable_all_gather_combine_by_dim=false --xla_gpu_enable_reduce_scatter_combine_by_dim=false --xla_disable_hlo_passes=rematerialization --xla_gpu_enable_command_buffer= --xla_gpu_experimental_enable_fusion_autotuner=false --xla_gpu_enable_allocator_spatial_partitioning=false"
else
  export XLA_FLAGS="--xla_gpu_memory_limit_slop_factor=95 --xla_gpu_reduce_scatter_combine_threshold_bytes=${COMBINE_TH_BYTES} --xla_gpu_enable_command_buffer='' --xla_gpu_enable_latency_hiding_scheduler=True --xla_gpu_all_gather_combine_threshold_bytes=${COMBINE_TH_BYTES} --xla_gpu_enable_triton_gemm=False --xla_gpu_enable_cublaslt=True --xla_gpu_autotune_level=${AUTOTUNE_LEVEL} --xla_gpu_enable_all_gather_combine_by_dim=FALSE"
fi
...
CMD=("$PYTHON_BIN" -m maxtext.trainers.pre_train.train "$MAXTEXT_CONFIG"
  ...
  "quantization=${QUANTIZATION}" "enable_nnx=${ENABLE_NNX}" "pure_nnx=${ENABLE_NNX}"
  "pure_nnx_decoder=${ENABLE_NNX}" "base_output_directory=${OUTDIR}")

print_recipe() {
  printf '%s\n' "=== RESOLVED_RECIPE_BEGIN ==="
  printf 'runner=performance recipe=%s mode=%s label=%s model=%s model_controls=%s processes=1 steps=%s measurement_window=completed_steps_40_49\n' "$RECIPE_PROFILE" "$MODE" "$LABEL" "$MODEL_NAME" "$MODEL_CONTROLS" "$STEPS"
  ...
  printf 'fsdp=%s shardy=%s scheduler=%s combine_threshold=%s enable_nnx=%s\n' "$FSDP" "$SHARDY" "$SCHEDULER_PROFILE" "$COMBINE_TH_BYTES" "$ENABLE_NNX"
  ...
  printf 'fp4_select=%s fp4_attention_gemm=%s fp4_mlp_gemm=%s bf16_hipblaslt_fallback=%s sr_key_mode=%s mha_fuse_gqa_reduce=%s mha_zero_pad=%s\n' "${FP4_SELECT:-n/a}" "${AITER_FP4_ATTN:-n/a}" "${AITER_FP4_MLP:-n/a}" "${AITER_BF16_HIPBLASLT:-n/a}" "$SR_KEY_MODE" "${JA_MHA_FUSE_GQA_REDUCE:-n/a}" "${JA_MHA_ZERO_PAD:-n/a}"
  ...
  printf 'rocm_safeguards=jax_platforms:%s,queue_interposition:%s,register_enabled:%s,no_scratch_reclaim:%s,dev_kernarg:%s,fine_grain_pcie:%s\n' "$JAX_PLATFORMS" "$ROCPROFILER_QUEUE_INTERPOSITION" "$ROCPROFILER_REGISTER_ENABLED" "${HSA_NO_SCRATCH_RECLAIM:-unset}" "$HIP_FORCE_DEV_KERNARG" "$HSA_FORCE_FINE_GRAIN_PCIE"
  printf 'rocm_sdk_library_path=%s\n' "$ROCM_SDK_LIBS"
...

{
  ...
  git -c safe.directory="$JAX_AITER_ROOT" -C "$JAX_AITER_ROOT" status --short --branch
  printf 'jax_aiter_git='
  git -c safe.directory="$JAX_AITER_ROOT" -C "$JAX_AITER_ROOT" rev-parse HEAD
  git -c safe.directory="$MAXTEXT_ROOT" -C "$MAXTEXT_ROOT" status --short --branch
  printf 'maxtext_git='
  git -c safe.directory="$MAXTEXT_ROOT" -C "$MAXTEXT_ROOT" rev-parse HEAD
  "$PYTHON_BIN" - <<'PY'
...
if [[ "$rc" == "0" ]]; then
  "$PYTHON_BIN" "$JAX_AITER_ROOT/scripts/validate_maxtext_train_log.py" \
    "$LOGFILE" --expected-last-step 49 --tail 10 \
    --out "$OUTDIR/parsed_tail.json"
fi
exit "$rc"

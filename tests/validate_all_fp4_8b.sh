#!/bin/bash
# All-FP4 training validation — Llama3-8B.
# Compares:
...
#   bash tests/validate_all_fp4_8b.sh            # 100 steps, 3 repeats
#   bash tests/validate_all_fp4_8b.sh 100 1      # 100 steps, 1 repeat (quick)
set -uo pipefail
...
LOGDIR=/ruvaidya/aiter_proj/docs/logs/all_fp4_8b
mkdir -p "$LOGDIR"
...
# Config order matters: FP8 is extremely sensitive to cumulative GPU thermal
# state. Running FP8 after 20+ minutes of prior training (as with several
# 100-step AITER configs back-to-back) silently drops FP8 steady-state from
# ~1,390 T/s to ~1,070 T/s at 8B, while the AITER configs themselves are
# largely unaffected. We therefore run FP8 FIRST in the sequence (on cool
# GPUs) so its measurement is thermally clean. The AITER configs are also
# less sensitive and show stable numbers regardless of position.
# See docs/logs/session20_e2e/SUMMARY.md "FP8 baseline" section for details.
...
# Per-run AITER_* env (semicolon-separated key=value; empty means unset).
declare -a CFG_ENV=(
  ""
...
echo "======================================================================"
echo "All-FP4 Validation — Llama3-8B"
...
for ((c=0; c<NUM_CFGS; c++)); do
    ...
    for ((r=1; r<=REPEATS; r++)); do
        ...
        EXTRA_ARGS="steps=$STEPS use_jax_aiter=$USE_AITER run_name=allfp4_8b_${TAG}"
        if [[ -n "$QUANT" ]]; then
...
echo ""
echo "======================================================================"
echo "ALL-FP4 8B RESULTS — ${STEPS} steps x ${REPEATS} repeats"
echo "======================================================================"
...
echo ""
echo "  Reference (session 18 production, 2026-04-11, 8x MI355X):"
echo "    Hybrid (FP4 attn + FP8 dB):  1,439 TFLOP/s  (+2.6% vs FP8)"
echo "    FP8 baseline:                 1,402 TFLOP/s"
echo ""
echo "  Promotion gate: all_fp4 must tie or beat hybrid_prod within 1% noise"
echo "  AND loss at step $STEPS within 0.03 of hybrid."
echo "======================================================================"

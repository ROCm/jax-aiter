...
// Compute dq_acc size for varlen layout.
static size_t compute_dq_acc_size_varlen(int64_t total_q, int64_t max_seqlen_k,
...
ffi::Error MhaVarlenBwd_Bridge(
  ...
  try {
    ...
    // Compute dq_acc strides for [split, total_q, nheads, head_dim].
    ...
    // nhead_stride_lse is the stride between heads.
    ...
    // Construct fmha_bwd_args for varlen; batch strides are zero.
    ...
    JA_LOG("MhaVarlenBwd completed successfully, elapsed time: %.3f ms",
           runtime);
    return ffi::Error::Success();

  } catch (const std::exception &e) {
    return ffi::Error(ffi::ErrorCode::kInternal,
                      std::string("mha_varlen_bwd: ") + e.what());
  }
}

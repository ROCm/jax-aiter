...
ffi::Error LLMM1_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
  ...
  JA_LOG("LLMM1 (%ld*%ld) . (%ld*%ld) -> (%ld*%ld) rows_per_block=%d", m, k, n,
         k, m, n, rows_per_block);
...
ffi::Error WvSplitK_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
                           ffi::AnyBuffer in_b,
  ...
  JA_LOG("WvSplitK (%ld*%ld) . (%ld*%ld) -> (%ld*%ld) N=%d CuCount=%d", m, k, k,
         n, m, n, N, CuCount);
  ...
  aiter::wvSpltK(A_t, B_t, out_t, static_cast<int64_t>(N),
                 static_cast<int64_t>(CuCount));
  return ffi::Error::Success();
}

ffi::Error WvSplitKSmall_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
                                ffi::AnyBuffer in_b,
                                ffi::Result<ffi::AnyBuffer> out_c, int32_t N,
                                int32_t CuCount) {
  ...
  JA_LOG("WvSplitKSmall (%ld*%ld) . (%ld*%ld) -> (%ld*%ld) N=%d CuCount=%d", m,
         k, k, n, m, n, N, CuCount);
  ...
  aiter::wv_splitk_small_fp16_bf16_wrapper(
      A_t, B_t, out_t, static_cast<int64_t>(N), static_cast<int64_t>(CuCount));
  return ffi::Error::Success();
}

ffi::Error WvSplitKQ_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
                            ffi::AnyBuffer in_b,
  ...
  JA_LOG("WvSplitKQ (%ld*%ld) . (%ld*%ld) -> (%ld*%ld) CuCount=%d", m, k, k, n,
         m, n, CuCount);
  ...
  aiter::wvSplitKQ(A_t, B_t, out_t, scale_a_t, scale_b_t,
                   static_cast<int64_t>(CuCount));
  return ffi::Error::Success();
}

ffi::Error LLZZ_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
                       ffi::AnyBuffer in_b, ffi::Result<ffi::AnyBuffer> out_c,
                       int32_t solidx) {
  ...
  JA_LOG("LLZZ (%ld*%ld) . (%ld*%ld) -> (%ld*%ld) solidx=%d", m, k, k, n, m, n,
         solidx);
...
ffi::Error MMCustomGPU_Bridge(hipStream_t stream, ffi::AnyBuffer in_a,
                              ffi::AnyBuffer in_b,
                              ffi::Result<ffi::AnyBuffer> out_c) {
  ...
  JA_LOG("MMCustomGPU (%ld*%ld) . (%ld*%ld) -> (%ld*%ld)", m, k, k, n, m, n);
